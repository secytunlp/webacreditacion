...
function isNumber(val, msg){
	if (!/^([0-9])*$/.test(val)) return msg; else return '';
...
function isImage(val, msg){
	var ext = val.split('.').pop().toLowerCase();
	var allow = new Array('gif','png','jpg','jpeg');
	if(jQuery.inArray(ext, allow) == -1) return msg; else return '';
	 
	}

function validate(idForm){
	
	if ( $('#'+idForm).jVal({style:'pod',padding:3,border:1,wrap:true}) )
		return true;	
	else
		return false;
}
...
(function($) {
        ...
        $.fn.jVal.defaults = {
                blurCheck: false,
                message: 'Entrada no v&aacute;lida',
                style: 'pod',
