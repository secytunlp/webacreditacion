...
if (PermisoQuery::permisosDeUsuario( $cd_usuario, 'Evaluar proyecto' )) {
	...
	$xtpl = new XTemplate ( 'evaluarproyecto.html' );
	...
	if ($oEvaluacion->getCd_estado()==6) {
		...
		switch ($subgrupo) {
					case 2:
					$speech = 'Evaluaci&oacute;n R (Regular) en este &iacute;tem, implica la NO APROBACION del Proyecto';
					$rowAlt ='<tr>';
					for($i = 0; $i < $count; $i ++) {	
						if ($i<3) {
							$rowAlt .='<td><div align="center">';
						}
						elseif ($i==4) $rowAlt .='<td colspan="2"><div align="center">Antecedentes';
						if ($i<4) 
							$rowAlt .='</div></td>';
					}
					$rowAlt .='</tr>';
					...
					$rowAlt ='<tr>';
					//for($i = 0; $i < $count; $i ++) {	
						$rowAlt .='<td colspan="3"><div align="center">Recursos para la Realizaci&oacute;n';
