...
if (PermisoQuery::permisosDeUsuario( $cd_usuario, 'Ver Observaciones SeCyT' )) {
	
	$xtpl = new XTemplate ( 'verobservaciones.html' );
	...
	for ($j = 0; $j < count($evaluaciones); $j++) {
		...
		$eval['ds_observaciones']=nl2br($oEvaluacion1->getDs_observacion());
		$eval['ds_observacionessecyt']=nl2br($oEvaluacion1->getDs_observacionsecyt());
		$eval['ds_evaluador']=$oUsuario->getDs_apynom();
	...
	$xtpl->assign ( 'titulo', 'SeCyT - Ver Evaluaciones' );
