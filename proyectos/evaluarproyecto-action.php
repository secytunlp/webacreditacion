...
if (PermisoQuery::permisosDeUsuario( $cd_usuario, 'Evaluar proyecto' )) {
	...
	$ultimoitem = EvaluacionproyectoplanillaQuery::ultimo_id();
	for($i = 1; $i <= $ultimoitem; $i ++) {	
		if ( isset($_POST ['nu_puntajeP'.$i]) ){
			...
			if ($oEvaluacionproyectopuntaje->getCd_evaluacionproyectopuntaje())
				EvaluacionproyectopuntajeQuery::modificarEvaluacionproyectopuntaje($oEvaluacionproyectopuntaje);
			else EvaluacionproyectopuntajeQuery::insertEvaluacionproyectopuntaje($oEvaluacionproyectopuntaje);
		}
	}
